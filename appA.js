...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('message', function (message) {
        ...
        socketMulticast(gara.get(socket.classe)[socket.livello],'message', {liv: socket.livello ,username: socket.username, punti: 2*socket.punti+socket.prove , message: message});
       ...
       // socket.emit('message', {liv: socket.livello ,username: socket.username,real:socket.real ,classe: socket.classe , punti: 2*socket.punti-socket.prove , message: message});       
        ...
        if (iscritti.get(classe).get(myid)){
            ...
            fs.writeFile('gara'+gare+'-'+classe+'.json', JSON.stringify(Object.fromEntries(iscritti.get(classe)), null, 2), 'utf-8', (err) => {
                if (err) {
