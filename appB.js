...
console.log(`Indirizzo IP locale del server: http://${serverIpAddress}:11911`);
...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('new_client', function(newname) {     
        ...
        console.log("classe:"+classe+"iscritti "+iscritti.get(classe).size);
        socketMulticast(gara.get(socket.classe)[socket.livello],'new_client',{username:socket.username,classe:socket.classe,real:socket.real, punti:0});
    ...
    socket.on('message', function (message) {
      ...
      //  socket.emit('message', {liv: socket.livello ,username: socket.username,real:socket.real ,classe: socket.classe , punti: 2*socket.punti-socket.prove , message: message}); 
       ...
       console.log('Aggiorno iscritti ');
        ...
        console.log('Avviso tutta la classe ',gara.get(socket.classe).size);
        socketMulticast(gara.get(socket.classe),'message', {liv: socket.livello ,username: socket.username,real: socket.real,classe: socket.classe , punti: 2*socket.punti-socket.prove , message: message});
